...
int zb_encode(unsigned char *destination, unsigned char device_number, 
		unsigned char command_number, int data)
{
	unsigned int i;
	...
	for (i = 2; i < 6; i++) 
	{
		destination[i] = (unsigned char)udata;
		udata >>= 8;
	}
...
int zb_decode(int *destination, unsigned char *reply)
{
	unsigned int i;
	unsigned int data = 0;
	...
	{
		*destination = -(int)(UINT32_MAX - data) - 1;
	}
...
int zb_send(z_port port, unsigned char *command)
{
	DWORD nbytes;
...
int zb_receive(z_port port, unsigned char *destination)
{
	DWORD nread;
	int i;
	char c;
...
int zb_send(z_port port, const unsigned char *command) 
{
	int nbytes;
