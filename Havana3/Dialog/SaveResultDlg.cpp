...
void SaveResultDlg::saveCrossSections()
{
	std::thread t1([&]() {
		...
		{			
			...
			if (m_pConfig->flimSyncInterFrame < intensity.size(1))
			{
				memcpy(&intensity(0, m_pConfig->flimSyncInterFrame), &intensity(0, 0), intensity.size(0) * (intensity.size(1) - m_pConfig->flimSyncInterFrame));
				memset(&intensity(0, 0), 0, intensity.size(0) * m_pConfig->flimSyncInterFrame);
			}

            int adjust = m_pVisTab->getFlimSyncAdjust();
			...
			if (m_pConfig->flimSyncInterFrame < lifetime.size(1))
			{
				memcpy(&lifetime(0, m_pConfig->flimSyncInterFrame), &lifetime(0, 0), lifetime.size(0) * (lifetime.size(1) - m_pConfig->flimSyncInterFrame));
				memset(&lifetime(0, 0), 0, lifetime.size(0) * m_pConfig->flimSyncInterFrame);
			}
	...
	std::thread t1([&]() {
		...
		if (checkList.bRaw)
		{
			for (int i = 0; i < 3; i++)
			{
				if (checkList.bCh[i])
				{
					...
					if (false != fileIntensity.open(QIODevice::WriteOnly))
					{
						...
						if (m_pConfig->flimSyncInterFrame < intensity_map.size(1))
						{
							memcpy(&intensity_map(0, m_pConfig->flimSyncInterFrame), &intensity_map(0, 0),
								sizeof(float) * intensity_map.size(0) * (intensity_map.size(1) - m_pConfig->flimSyncInterFrame));
							memset(&intensity_map(0, 0), 0, sizeof(float) * intensity_map.size(0) * m_pConfig->flimSyncInterFrame);
						}
						int adjust = m_pVisTab->getFlimSyncAdjust();
						for (int i = 0; i < intensity_map.size(1); i++)
						{
							float* pImg = intensity_map.raw_ptr() + i * intensity_map.size(0);
							std::rotate(pImg, pImg + adjust, pImg + intensity_map.size(0));
						}

						fileIntensity.write(reinterpret_cast<char*>(intensity_map.raw_ptr()), sizeof(float) * intensity_map.length());
						fileIntensity.close();
					}
					...
					if (false != fileLifetime.open(QIODevice::WriteOnly))
					{
						...
						if (m_pConfig->flimSyncInterFrame < lifetime_map.size(1))
						{
							memcpy(&lifetime_map(0, m_pConfig->flimSyncInterFrame), &lifetime_map(0, 0),
								sizeof(float) * lifetime_map.size(0) * (lifetime_map.size(1) - m_pConfig->flimSyncInterFrame));
							memset(&lifetime_map(0, 0), 0, sizeof(float) * lifetime_map.size(0) * m_pConfig->flimSyncInterFrame);
						}
						int adjust = m_pVisTab->getFlimSyncAdjust();
						for (int i = 0; i < lifetime_map.size(1); i++)
						{
							float* pImg = lifetime_map.raw_ptr() + i * lifetime_map.size(0);
							std::rotate(pImg, pImg + adjust, pImg + lifetime_map.size(0));
						}

						fileLifetime.write(reinterpret_cast<char*>(lifetime_map.raw_ptr()), sizeof(float) * lifetime_map.length());
						fileLifetime.close();
					}
				}
			}
			...
			if (checkList.bOctProj)
			{
				...
				{
					fileOctMaxProj.write(reinterpret_cast<char*>(m_pVisTab->m_octProjection.raw_ptr()), sizeof(uint8_t) * m_pVisTab->m_octProjection.length());
		...
		if (checkList.bScaled)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (checkList.bCh[i])
				{
					...
					ImageObject imgObjLifetime(roi_flimproj.width, roi_flimproj.height, temp_ctable.m_colorTableVector.at(m_pConfig->flimLifetimeColorTable));
					...
					if (m_pConfig->flimSyncInterFrame < imgObjIntensity.arr.size(1))
					{
						memcpy(&imgObjIntensity.arr(0, 0), &imgObjIntensity.arr(0, m_pConfig->flimSyncInterFrame),
							imgObjIntensity.arr.size(0) * (imgObjIntensity.arr.size(1) - m_pConfig->flimSyncInterFrame));
						memset(&imgObjIntensity.arr(0, imgObjIntensity.arr.size(1) - m_pConfig->flimSyncInterFrame), 0,
							imgObjIntensity.arr.size(0) * m_pConfig->flimSyncInterFrame);
					}
					...
					for (int j = 0; j < 2; j++)
					{
						...
						if (m_pConfig->flimSyncInterFrame < imgObjIntensityRatio.arr.size(1))
						{
							memcpy(&imgObjIntensityRatio.arr(0, 0), &imgObjIntensityRatio.arr(0, m_pConfig->flimSyncInterFrame),
								imgObjIntensityRatio.arr.size(0) * (imgObjIntensityRatio.arr.size(1) - m_pConfig->flimSyncInterFrame));
							memset(&imgObjIntensityRatio.arr(0, imgObjIntensityRatio.arr.size(1) - m_pConfig->flimSyncInterFrame), 0,
								imgObjIntensityRatio.arr.size(0) * m_pConfig->flimSyncInterFrame);
						}
						...
						imgObjIntensityRatio.qindeximg.save(enFacePath + QString("intensity_ratio_ch%1_%2_[%3 %4].bmp").arg(num + 1).arg(den + 1)
					...
					if (m_pConfig->flimSyncInterFrame < imgObjLifetime.arr.size(1))
					{
						memcpy(&imgObjLifetime.arr(0, 0), &imgObjLifetime.arr(0, m_pConfig->flimSyncInterFrame),
							imgObjLifetime.arr.size(0) * (imgObjLifetime.arr.size(1) - m_pConfig->flimSyncInterFrame));
						memset(&imgObjLifetime.arr(0, imgObjLifetime.arr.size(1) - m_pConfig->flimSyncInterFrame), 0,
							imgObjLifetime.arr.size(0) * m_pConfig->flimSyncInterFrame);
					}
			...
			if (checkList.bOctProj)
			{
				...
				ImageObject imgObjOctMaxProj(roi_proj.width, roi_proj.height, temp_ctable.m_colorTableVector.at(m_pConfig->octColorTable));
				...
				imgObjOctMaxProj.qindeximg.save(enFacePath + QString("oct_max_projection_gray[%1 %2].bmp").arg(m_pConfig->octGrayRange.min).arg(m_pConfig->octGrayRange.max), "bmp");
			}
		}
	...
	std::vector<np::Uint8Array2>& intensityMap, std::vector<np::Uint8Array2>& lifetimeMap, CrossSectionCheckList checkList)
{
	...
	while (frameCount < nTotalFrame)
	{
		...
		pImgObjVec->push_back(new ImageObject(roi_oct.height, roi_oct.width, temp_ctable.m_colorTableVector.at(m_pConfig->octColorTable)));
		...
		pImgObjVec->push_back(new ImageObject(roi_oct.height / 4, RING_THICKNESS, temp_ctable.m_colorTableVector.at(m_pConfig->flimLifetimeColorTable)));
	...
	while (frameCount < nTotalFrame)
	{
		...
		if (!checkList.bCh[0] && !checkList.bCh[1] && !checkList.bCh[2])
		{
			...
			ImageObject *pCircImgObj = new ImageObject(2 * m_pVisTab->m_vectorOctImage.at(frameCount).size(0), 2 * m_pVisTab->m_vectorOctImage.at(frameCount).size(0), 
				temp_ctable.m_colorTableVector.at(m_pConfig->octColorTable));
