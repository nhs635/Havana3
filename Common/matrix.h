/*
 ...
 you can apply operations with matrices and floats:
 A = B + C;
 A = B - C;
 A = -B;
 A = B * C;
...
public:
    ...
    {
        if (p != nullptr && r > 0 && r <= rows && c > 0 && c <= cols)
        {
            return p[r-1][c-1];
        }
        else
    ...
    // add a float value (elements wise)
    ...
    // subtract a float value (elements wise)
    Matrix& Subtract(const float v)
    {
        return Add(-v);
    }
    
    // multiply a float value (elements wise)
    ...
    // divide a float value (elements wise)
    ...
    // subtraction of Matrix with float
    friend Matrix operator- (const Matrix& a, const float b)
    {
        Matrix res = a;
        res.Subtract(b);
    ...
    // subtraction of float with Matrix
    friend Matrix operator- (const float b, const Matrix& a)
    {
        Matrix res = -a;
        res.Add(b);
    ...
    // multiplication of Matrix with float
    friend Matrix operator* (const Matrix& a, const float b)
    {
        Matrix res = a;
        res.Multiply(b);
    ...
    // multiplication of float with Matrix
    friend Matrix operator* (const float b, const Matrix& a)
    {
        Matrix res = a;
        res.Multiply(b);
...
private:
    int rows;
    int cols;
    float** p;     // pointer to a matrix with floats
};
...
inline int Size(const Matrix& a, const int i)
{
    return a.Size(i);
}
 ...
 */
inline Matrix Ones(const int rows, const int cols)
{
 ...
 */
inline Matrix Zeros(const int rows, const int cols)
{
 ...
 */
inline Matrix Diag(const int n)
{
 ...
 */
inline Matrix Diag(const Matrix& v)
{
    Matrix res;
    if (v.GetCols() == 1)
    {
...
{
    float d = 0;    // value of the determinant
    int rows = a.GetRows();
    int cols = a.GetRows();
